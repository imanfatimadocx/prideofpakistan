...
function getEmbedUrl(input: string): string {
  ...
  if (
    ...
    input.includes("vimeo.com/video/")
  )
    return input;
  return input;
}
...
export default function MediaAdminClient({
  ...
  async function handleSave() {
    ...
    if (!form.url.trim()) {
      setError("YouTube URL or embed code is required.");
      return;
    }
  ...
  async function toggleFeatured(v: Video) {
    ...
    setVideos((prev) =>
      prev.map((vid) =>
        vid.video_id === v.video_id
          ? { ...vid, featured: featured ? "feature" : "no" }
          : vid,
      ),
    );
  }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="p-6 space-y-5 bg-white border border-border rounded-xl">
        ...
        {preview && (
          <div>
            <p className="mb-2 text-xs font-semibold tracking-wide uppercase text-ink-muted font-body">
              Thumbnail Preview
            </p>
        ...
        <label className="flex items-center gap-3 cursor-pointer select-none">
          ...
          >
            <div
              className={`w-4 h-4 bg-white rounded-full shadow transition-transform mx-0.5 ${form.featured ? "translate-x-5" : "translate-x-0"}`}
            />
          </button>
          <div>
                ...
                <div className="flex items-center gap-4 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      {v.featured === "feature" && (
                        <span className="text-[10px] font-bold text-gold bg-gold-pale px-2 py-0.5 rounded font-body flex-shrink-0">
                          Featured
                        </span>
                      )}
                    </div>
                    <p className="text-xs text-ink-muted font-body">
                      Added{" "}
                      {new Date(v.datetime).toLocaleDateString("en-GB", {
                        day: "numeric",
                        month: "short",
                        year: "numeric",
                      })}
                  ...
                  <div className="flex flex-wrap items-center justify-end flex-shrink-0 gap-3">
                    <span
                      className={`text-[11px] font-bold px-2.5 py-0.5 rounded border font-body ${
                        v.status === "active"
                    ...
                    <button
                      ...
                      className="text-xs transition-colors text-ink-muted font-body hover:text-gold"
                    >
                      {v.featured === "feature" ? "Unfeature" : "Feature"}
                    </button>
                    <button
                      onClick={() => toggleStatus(v)}
                      className="text-xs transition-colors text-ink-muted font-body hover:text-green"
                    >
