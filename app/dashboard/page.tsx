    ...
    <main className="min-h-screen py-10 bg-cream sm:py-14">
      <div className="max-w-[1100px] mx-auto px-4 sm:px-8 lg:px-12">
        ...
        <div className="mb-8">
          ...
          <h1 className="mb-1 text-3xl font-bold font-display text-green">
            Welcome, {session.user?.name ?? "there"}
          </h1>
          <p className="text-sm text-ink-muted font-body">
            Manage your submissions and contributions to Pride of Pakistan.
          </p>
        </div>
        ...
        {submitted && (
          <div className="p-4 mb-6 text-sm border rounded-lg bg-green/10 border-green/20 font-body text-green">
            {submitted === "story" &&
              "Your story has been submitted and is pending review."}
            {submitted === "profile" &&
              "Your profile has been submitted and is pending review."}
            {submitted === "business" &&
              "Your business listing has been submitted and is pending review."}
          </div>
        )}
          ...
          {[
            {
              ...
              href: "/submit-profile",
              desc: "Add yourself to Who Is Who",
            },
            {
              label: "List Business",
              href: "/list-business",
              desc: "Add to the businesses",
            },
            {
              label: "Share a Story",
              href: "/your-stories/submit",
              desc: "Write for Your Stories",
            },
          ].map((a) => (
            <Link
              key={a.href}
              href={a.href}
              className="bg-white border border-border rounded-xl p-4 no-underline hover:border-gold hover:-translate-y-0.5 transition-all"
            >
              <p className="mb-1 text-sm font-bold text-ink-dark font-body">
                {a.label}
              </p>
        ...
        <div className="overflow-hidden bg-white border border-border rounded-xl">
          ...
          {/* Content */}
          {loading ? (
            <div className="p-8 text-sm text-center text-ink-muted font-body">
            ...
            <div className="p-8 text-center">
              ...
              <p className="mt-1 text-xs text-ink-muted font-body">
                Use the quick actions above to make your first submission.
              </p>
            </div>
          ) : (
              ...
              {filtered.map((s) => (
                ...
                >
                  <div className="flex items-center min-w-0 gap-3">
                    ...
                    <p className="text-sm font-medium truncate text-ink-dark font-body">
                      {s.title}
                    </p>
                  </div>
                  <div className="flex items-center flex-shrink-0 gap-3">
                    <span
                      className={`text-[11px] font-bold px-2.5 py-0.5 rounded border font-body ${STATUS_STYLES[s.status] ?? STATUS_STYLES.pending}`}
                    >
