...
export default function NewsStrip({ news = [] }: Props) {
  ...
  return (
    <section
      className="py-12 bg-white border-t sm:py-16 lg:py-18 border-border"
      id="top-stories"
    >
              ...
              {item.smallimage && (
                <div className="w-full overflow-hidden h-44 sm:h-48">
                  ...
                  <img
                    src={item.smallimage}
                    alt={item.title}
                    className="object-fit w-full h-full transition-transform duration-400 group-hover:scale-105"
                  />
                </div>
              )}
